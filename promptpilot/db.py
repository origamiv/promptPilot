...
def _find_ref_task_status_id_by_shortname(cur, shortname: str, *, active_only: bool = False) -> Optional[int]:
    """Pick reference task status ID by shortname (optionally active only)."""
    key = str(shortname or "").strip().lower()
    if not key:
        return None
    query = """
        SELECT id
        FROM {}.tasks_statuses
        WHERE LOWER(shortname) = %s
    """
    params = [key]
    if active_only:
        query += " AND status = 1"
    query += """
        ...
        LIMIT 1
    """
    cur.execute(sql.SQL(query).format(sql.Identifier(SCHEMA_NAME)), tuple(params))
    row = cur.fetchone()
    return int(row["id"]) if row else None
...
def init_db():
    with _connect() as conn, conn.cursor() as cur:
        ...
        # Insert backlog status as first column and shift existing nom values once.
        cur.execute(
            sql.SQL(
                """
                UPDATE {}.tasks_statuses
                SET nom = COALESCE(nom, 0) + 1
                WHERE shortname <> 'backlog'
                  AND NOT EXISTS (
                    SELECT 1
                    FROM {}.tasks_statuses
                    WHERE shortname = 'backlog'
        ...
        cur.execute(
            sql.SQL(
                """
                INSERT INTO {}.tasks_statuses (nom, name, shortname, status_to, color, status, created_at, updated_at)
                VALUES (8, 'Готово', 'ready', 'completed', '#10b981', 0, NOW(), NOW())
                ON CONFLICT (shortname) DO UPDATE
                SET nom = EXCLUDED.nom,
                    name = EXCLUDED.name,
                    status_to = EXCLUDED.status_to,
        ...
        cur.execute(
            sql.SQL(
                "CREATE INDEX IF NOT EXISTS {} ON {}.skills (is_active)"
            ).format(
                sql.Identifier("idx_skills_is_active"),
                sql.Identifier(SCHEMA_NAME),
            )
        )
        ...
        cur.execute(
            sql.SQL(
                """
                INSERT INTO {}.mcps (name, shortname, description, command, args, created_at, updated_at)
                VALUES
            ...
            (
                "Context7", "context7", "Документация библиотек через Context7", "npx",
                json.dumps(["-y", "@upstash/context7-mcp"]),
                "iMessage", "imessage", "Интеграция с iMessage", "bun",
                json.dumps(["run", "--cwd", "/home/agent/.claude/plugins/marketplaces/claude-plugins-official/external_plugins/imessage", "--shell=bun", "--silent", "start"]),
                "Discord", "discord", "Интеграция с Discord", "bun",
                json.dumps(["run", "--cwd", "/home/agent/.claude/plugins/marketplaces/claude-plugins-official/external_plugins/discord", "--shell=bun", "--silent", "start"]),
                "Telegram", "telegram", "Интеграция с Telegram", "bun",
                json.dumps(["run", "--cwd", "/home/agent/.claude/plugins/marketplaces/claude-plugins-official/external_plugins/telegram", "--shell=bun", "--silent", "start"]),
                "Fake Chat", "fakechat", "Фейковые чаты для тестирования", "bun",
                json.dumps(["run", "--cwd", "/home/agent/.claude/plugins/marketplaces/claude-plugins-official/external_plugins/fakechat", "--shell=bun", "--silent", "start"]),
                "Linear", "linear", "Интеграция с Linear (таск-трекер)", "bun",
                json.dumps(["run", "--cwd", "/home/agent/.claude/plugins/marketplaces/claude-plugins-official/external_plugins/linear", "--shell=bun", "--silent", "start"]),
                "YouGile", "yougile", "Интеграция с YouGile (таск-трекер)", "npx",
                json.dumps(["-y", "@yougile/mcp"]),
            ),
        )
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                ...
                FROM {}.skills
                WHERE (%s::text IS NULL OR name ILIKE %s::text OR description ILIKE %s::text)
                  AND (%s::text IS NULL OR agents @> to_jsonb(%s::text))
                  AND (NOT %s::boolean OR is_active = TRUE)
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (q_param, q_param, q_param, agent, agent, active_only),
        )
        total = (cur.fetchone() or {}).get("total", 0)
        cur.execute(
            sql.SQL(
                """
                SELECT id, name, slug, description, argument_hint, agents, is_active, created_at, updated_at
                FROM {}.skills
                WHERE (%s::text IS NULL OR name ILIKE %s::text OR description ILIKE %s::text)
                  AND (%s::text IS NULL OR agents @> to_jsonb(%s::text))
                  AND (NOT %s::boolean OR is_active = TRUE)
            ...
            (q_param, q_param, q_param, agent, agent, active_only, page_size, offset),
        )
        items = cur.fetchall()
...
def create_skill(
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                """
                INSERT INTO {}.skills (name, slug, description, content, argument_hint, agents, is_active, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s::jsonb, %s, NOW(), NOW())
                RETURNING *
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (name, slug, description, content, argument_hint, json.dumps(agents), is_active),
        )
        return cur.fetchone()
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                """
                UPDATE {}.skills
                SET name = %s, slug = %s, description = %s, content = %s,
                    argument_hint = %s, agents = %s::jsonb, is_active = %s, updated_at = NOW()
                WHERE id = %s
                RETURNING *
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (name, slug, description, content, argument_hint, json.dumps(agents), is_active, skill_id),
        )
        return cur.fetchone()


def toggle_skill(skill_id: int, is_active: Optional[bool] = None) -> Optional[dict]:
    with _connect() as conn, conn.cursor() as cur:
...
def delete_skill(skill_id: int) -> Optional[str]:
    """Delete skill and return its slug (or None if not found)."""
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                "DELETE FROM {}.skills WHERE id = %s RETURNING slug"
            ).format(sql.Identifier(SCHEMA_NAME)),
            (skill_id,),
        )
        row = cur.fetchone()
...
def list_skills_for_provider(provider: str) -> list:
    """Return active skills available for a given provider (shortname)."""
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                """
                SELECT name, description, argument_hint
                FROM {}.skills
                WHERE is_active = TRUE
                  AND (agents = '[]'::jsonb OR agents @> to_jsonb(%s::text))
                ORDER BY name ASC
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (provider,),
...
def list_mcps(search: Optional[str] = None, limit: int = 200) -> list[dict]:
    with _connect() as conn, conn.cursor() as cur:
        if search:
            like = f"%{search.strip()}%"
            cur.execute(
                sql.SQL(
                    """
                    SELECT id, name, shortname, description, command, args, env, options, status, created_at, updated_at
                    FROM {}.mcps
                    WHERE name ILIKE %s OR shortname ILIKE %s
                    ORDER BY id ASC
                    LIMIT %s
...
def get_mcp(mcp_id: int) -> Optional[dict]:
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                """
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                """
                INSERT INTO {}.mcps
                    (name, shortname, description, command, args, env, options, status, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s::jsonb, %s::jsonb, %s::jsonb, %s, %s, %s)
                RETURNING id, name, shortname, description, command, args, env, options, status, created_at, updated_at
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (
                name, shortname, description, command,
...
def update_mcp(mcp_id: int, **kwargs) -> bool:
    allowed = {"name", "shortname", "description", "command", "args", "env", "options", "status"}
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL("UPDATE {}.mcps SET {} WHERE id = %s").format(
                sql.Identifier(SCHEMA_NAME),
...
def delete_mcp(mcp_id: int) -> bool:
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL("DELETE FROM {}.mcps WHERE id = %s").format(sql.Identifier(SCHEMA_NAME)),
            (mcp_id,),
        )
        return cur.rowcount > 0
