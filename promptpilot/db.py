            ...
            cur.execute(
                sql.SQL(
                    ...
                    SELECT aa.id, aa.name, aa.shortname, aa.agent_id, a.name AS agent_name,
                           aa.login, aa.pass, aa.token, aa.login_mode, aa.created_at, aa.updated_at
        ...
        cur.execute(
            sql.SQL(
                ...
                INSERT INTO {}.agents_accounts
                    (name, shortname, agent_id, login, pass, token, login_mode, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING id, name, shortname, agent_id, login, pass, token, login_mode, created_at, updated_at
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (name, shortname, agent_id, login, password, token, login_mode, now, now),
        )
        return dict(cur.fetchone())
...
def update_agent_account(
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            (name, shortname, agent_id, login, password, token, login_mode, now, account_id),
        )
        return cur.rowcount > 0
    ...
    with _connect() as conn, conn.cursor() as cur:
        if search:
            ...
            cur.execute(
                sql.SQL(
                    """
                    SELECT id, name, shortname, message, options, created_at, updated_at
                    FROM {}.prompts
...
def create_prompt(name: str, shortname: str, message: str, options: Optional[dict] = None) -> dict:
    now = datetime.utcnow().replace(microsecond=0)
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                """
                INSERT INTO {}.prompts
                    (name, shortname, message, options, created_at, updated_at)
                VALUES (%s, %s, %s, %s::jsonb, %s, %s)
                RETURNING id, name, shortname, message, options, created_at, updated_at
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (name, shortname, message, json.dumps(options) if options is not None else None, now, now),
        )
        return dict(cur.fetchone())
...
def update_prompt(
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            (name, shortname, message, json.dumps(options) if options is not None else None, now, prompt_id),
        )
        return cur.rowcount > 0
