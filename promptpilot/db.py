...
def init_db():
    with _connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            sql.SQL(
                """
                INSERT INTO {}.tasks_statuses (id, name, shortname, status_to, color, status, created_at, updated_at)
                VALUES
                  (1, 'Успешно завершена',        'done',             'completed', '#22c55e', 1, NOW(), NOW()),
                  (2, 'Завершена с ошибкой',       'failed',           'failed',    '#ef4444', 1, NOW(), NOW()),
                  (3, 'В работе',                  'in_progress',      'running',   '#3b82f6', 1, NOW(), NOW()),
                  (4, 'Новая',                     'new',              'pending',   '#6b7280', 1, NOW(), NOW()),
                  (5, 'Ожидает подтверждения',     'pending_approval', NULL,        '#f59e0b', 1, NOW(), NOW()),
                  (6, 'Ожидает тестирования',      'pending_test',     NULL,        '#8b5cf6', 1, NOW(), NOW())
    ...
    with _connect() as conn, conn.cursor() as cur:
        if search:
            ...
            cur.execute(
                sql.SQL(
                    """
                    SELECT id, name, shortname, status_to, color, status, created_at, updated_at
                    FROM {}.tasks_statuses
        ...
        cur.execute(
            sql.SQL(
                ...
                INSERT INTO {}.tasks_statuses
                    (name, shortname, status_to, color, status, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                RETURNING id, name, shortname, status_to, color, status, created_at, updated_at
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (name, shortname, status_to, color, status, now, now),
        )
        return dict(cur.fetchone())
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                ...
                SET name = %s, shortname = %s, status_to = %s, color = %s, status = %s, updated_at = %s
                WHERE id = %s
                """
            ).format(sql.Identifier(SCHEMA_NAME)),
            (name, shortname, status_to, color, status, now, status_id),
        )
        return cur.rowcount > 0
